...
```{r}
Irlanda <- Irlanda[!is.na(Irlanda$BEINGBULLIED), ]
```
...
### Mother
# Mother <- c("BMMJ1", "MISCED")
# Irlanda <- Irlanda[, -which(names(Irlanda) %in% Mother)]
# Irlanda
### Father
#### HISEI - Index highest parental occupational status (se calhar poderiamos criar uma nova variável que é a soma das 2 apagadas?)
# Father <- c("BFMJ2", "FISCED")
# Irlanda <- Irlanda[, -which(names(Irlanda) %in% Father)]

## Number of classes (usar o total apenas)
Class <- c("ST059Q03TA", "ST059Q02TA", "ST059Q01TA", "ST060Q01NA")
Irlanda <- Irlanda[, -which(names(Irlanda) %in% Class)]

## Learning Time
LearningTime <- c("MMINS", "LMINS", "SMINS", "TMINS")
Irlanda <- Irlanda[, -which(names(Irlanda) %in% LearningTime)]

## Variáveis deixadas de lado, pois, ou não encotramos informação sonbre elas ou não seria úteis
### ST001D01T possui o ano escolar (8º, 9º, 10º, 11º) em que cada um dos aluno está
### ISCEDL só possui 2 e 3, que é associado ao ensino secundário "baixo" e "alto"
### Retiradas pelo seu valor fraco
# - REPEAT
# - INFOJOB2
# - SOCONPA
# - PRESUPP
# - ATTLNACT
# - IMMIG
Resto <- c("AGE", "ST001D01T", "ISCEDL", "SOIAICT", "AUTICT", "STUBMI", "INFOJOB2", "SOCONPA", "ATTLNACT", "IMMIG", "DISCLIMA", "ICTSCH", "ICTHOME", "JOYREADP")
Irlanda <- Irlanda[, -which(names(Irlanda) %in% Resto)]
...
low_kmo_variables <- names(kmo_result$MSAi)[kmo_result$MSAi < 0.6]
low_kmo_variables
Irlanda <- Irlanda[, !names(Irlanda) %in% low_kmo_variables]
```
...
```{r}
pc12 <- principal(data_scaled, nfactors = 13, rotate = "none")
pc12$loadings
round(pc12$communality,2)
```

```{r}
pc12r <- principal(data_scaled, nfactors= 13, rotate="varimax")
pc12r$loadings
round(pc12r$communality,2)
#RC10 pode ser caracteristicas dos pais
```
