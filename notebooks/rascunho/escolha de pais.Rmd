...
Class <- c("ST059Q03TA", "ST059Q02TA", "ST059Q01TA")
Irlanda <- Irlanda[, -which(names(Irlanda) %in% Class)]

## Learning Time
LearningTime <- c("MMINS", "LMINS", "SMINS")
...
# - ATTLNACT
# - IMMIG
Resto <- c("AGE", "ST001D01T", "ISCEDL", "SOIAICT", "AUTICT", "BEINGBULLIED", "STUBMI", "REPEAT", "INFOJOB2", "SOCONPA", "PRESUPP", "ATTLNACT")
...
low_kmo_variables <- names(kmo_result$anti.image)[kmo_result$anti.image < 0.5]
low_kmo_variables
Irlanda <- Irlanda[, !names(Irlanda) %in% low_kmo_variables]
# Irlanda <- Irlanda[, -which(names(Irlanda) %in% low_kmo_variables)]
# low_kmo_variables
```
...
```{r}
pc12sc <- principal(data_scaled, nfactors=10, rotate="none", scores = TRUE)
