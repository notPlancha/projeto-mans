...
Irlanda <- IrlandaImputInc[, -which(names(IrlandaImputInc) %in% Perfil)]
```
...
# Neste output o LMINS, o MMINS, o ST059Q02TA, o ST059Q01TA, o HISEI e o HISCED tem valores abaixo de 0.6 (0.27, 0.23, 0.22, 0.25, 0.42, 0.39, respetivamente). É chill ou é para retirar estas variáveis também?
```
...
### RC1 - Alegria e bem-estar interior
...
> Comparativamente ao ficheiro sem Imputação, o RC5 e o RC1 estão ao contrário
...
```{r}
IrlandaImput$Intelligence <- pc6sc$scores[,1] # RC2
IrlandaImput$Socioeconomic <- pc6sc$scores[,2] # RC3
IrlandaImput$Hapiness <- pc6sc$scores[,3] # RC1
IrlandaImput$SchoolSupport <- pc6sc$scores[,4] # RC5
IrlandaImput$CareerAwareness <- pc6sc$scores[,5] # RC4
IrlandaImput$ICTKnowledge <- pc6sc$scores[,6] # RC6
```
...
```{r}
pc_dist <- dist(IrlandaImput[,43:47])
...
wssplot(IrlandaImput[,43:47], nc=10)
```

```{r}
#K-means cluster com K=4
kmeans.k6 <- kmeans(IrlandaImput[,43:47], 4, nstart=100)
IrlandaImput = IrlandaImput %>% mutate(cluster = kmeans.k6$cluster)
```

```{r}
table(groups.k5, IrlandaImput$cluster)
# O 2 e o 3 podem estar trocados
```

```{r}
#Barplot of average score in each principal component within each cluster
barplot(colMeans(subset(IrlandaImput,cluster==1)[,43:47]),main= "Cluster 1 - Average score in each principal component")
barplot(colMeans(subset(IrlandaImput,cluster==2)[,43:47]),main= "Cluster 2 - Average score in each principal component")
barplot(colMeans(subset(IrlandaImput,cluster==3)[,43:47]),main= "Cluster 3 - Average score in each principal component")
barplot(colMeans(subset(IrlandaImput,cluster==4)[,43:47]),main= "Cluster 4 - Average score in each principal component")
# barplot(colMeans(subset(IrlandaImput,cluster==5)[,44:48]),main= "Cluster 4 - Average score in each principal component")
```

```{r}
std_data <- scale(IrlandaImput[,43:47])
...
```{r}
# select VVE -> Variable Volume, Variable shape, equal directions
