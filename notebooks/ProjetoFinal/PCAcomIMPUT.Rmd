...
valores
```
Visualizando os valores acima, podemos então concluir que existem ```{r} length(valores[valores > 1]) ``` PCA's recomendados, pois, possuem um valor superior a 1. Para uma melhor escolha de PCA's, iremos visualizar os valores do gráfico, a fim de escolher o melhor número de PCA's possivel.
...
```
RC1 - Empenho do aluno:
  - Boas Notas
  - Conhecimento tecnológico
  - Não usa muitas tecnologias, pois, na irlanda não existe um grande intensivo em relação ao uso de tecnologias de forma académica.
  ![Uso de ITC](ITC.png)
  -
```{r}
...
```{r}
# RC1 - Empenho do aluno (Boas notas)
```
