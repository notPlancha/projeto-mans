...
colunas_com_nulos <- colnames(nulos_percent)[nulos_percent > 15]
colunas_com_nulos
```
...
x <-Irlanda[, sapply(Irlanda, is.numeric)] %>%
  my_skim() %>%
...
tabela_html <- htmlTable(x, align = "c", caption = 'Descriptives with histogram')
tabela_html
```

### Extra

Sabendo que o objetivo é criar PCA's, iremos já retirar os dados de perfil

```{r}
Perfil <- c("ST004D01T", "AGE")
Irlanda <- Irlanda[, -which(names(Irlanda) %in% Perfil)]
```
