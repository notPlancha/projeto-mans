...
```{r}
# A variável GRADE faz com que o teste KMO dê erro (não se sabe)
# Aqui serão apagadas todas as variáveis que, ou podem ser substituidas, ou não tem valor nenhum

Apagar <- c("GRADE", "JOYREADP", "PRESUPP", "PQSCHOOL", "EMOSUPP", "CURSUPP", "INFOJOB2", "ICTOUTSIDE", "ICTCLASS", "INTICT", "ENTUSE", "ATTLNACT", "DISCLIMA", "ICTSCH", "ICTSCH", "REPEAT", "IMMIG", "FISCED", "MISCED", "PASCHPOL", "COMPETE", "SMINS", "ST061Q01NA", "ST060Q01NA", "ST059Q03TA", "PERCOMP", "CHANGE", "SCCHANGE", "BFMJ2", "BMMJ1")
...
### RC5 - Alegria e bem-estar interior
...
### RC1 - Apoio por parte do professor
...
### RC4 - Nível de educação e informação sobre carreiras
...
### RC6 - Manuseamento de ICT
- Esta componente principal tem em conta o uso de ICT por parte do estudante no que diz respeito a integração social bem como a autonomia do mesmo com estas tecnologias. Esta componente principal também tem em conta a percepção de competência relativamente ao manuseamento de ICT por parte do estudante.

> Comparativamente ao ficheiro com Imputação, o RC5 e o RC1 estão ao contrário
...
- - -
```{r}
pc6sc <- principal(data_scaled, nfactors = 6, rotate = "equamax", scores = TRUE)
...
```{r}
IrlandaNoImput$Intelligence <- pc6sc$scores[,1] # RC2 - Resultados Escolares
IrlandaNoImput$Socioeconomic <- pc6sc$scores[,2] # RC3 - Estatuto socio-económico
IrlandaNoImput$SchoolSupport <- pc6sc$scores[,3] # RC1 - Apoio por parte do professor
IrlandaNoImput$Hapiness <- pc6sc$scores[,4] # RC5 - Alegria e bem-estar interior 
IrlandaNoImput$CareerAwareness <- pc6sc$scores[,5] # RC4 - Nível de educação e informação sobre carreiras
IrlandaNoImput$ICTKnowledge <- pc6sc$scores[,6] # RC6 - Manuseamento de ICT
```
...
```{r}
IrlandaNoImput[,43:47]
```

```{r}
corr <- cor(IrlandaNoImput[,43:47])
...
```{r}
pc_dist <- dist(IrlandaNoImput[,43:47])
...
wssplot(IrlandaNoImput[,43:47], nc=10)
```

```{r}
#K-means cluster com K=4
kmeans.k6 <- kmeans(IrlandaNoImput[,43:47], 4, nstart=100)
...
barplot(colMeans(subset(IrlandaNoImput,cluster==1)[,43:47]),main= "Cluster 1 - Average score in each principal component")
barplot(colMeans(subset(IrlandaNoImput,cluster==2)[,43:47]),main= "Cluster 2 - Average score in each principal component")
barplot(colMeans(subset(IrlandaNoImput,cluster==3)[,43:47]),main= "Cluster 3 - Average score in each principal component")
barplot(colMeans(subset(IrlandaNoImput,cluster==4)[,43:47]),main= "Cluster 4 - Average score in each principal component")
# barplot(colMeans(subset(IrlandaNoImput,cluster==5)[,44:48]),main= "Cluster 4 - Average score in each principal component")
```

```{r}
std_data <- scale(IrlandaNoImput[,43:47])
...
```{r}
# select VVI -> Variable Volume, Variable shape, Coordinate axes
