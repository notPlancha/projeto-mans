...
IrlandaImput %>%   arrow::write_csv_arrow(here("data", "IrlandaImput.csv"))

# Base de dados com os dados omissos eliminados
IrlandaNoImput %>% arrow::write_csv_arrow(here("data", "IrlandaNoImput.csv"))
```
...
Irlanda %>%
  ...
  mutate(First_5_Values = sapply(Column, \(x) paste(head(na.omit(dados[[x]]), 5) , collapse = ", "))) %>% mutate(naPercent = sapply(Column, \(x) Irlanda %>% zap_labels() %>% .[[x]] %>% is.na() %>% mean %>% "*"(100) %>% round(2))) -> dfI.desc
dfI.desc %>% View()
dfI.desc
```
